...
namespace bgen {
    namespace parser{
        namespace actions {
            ...
            void inplace_struct_action (context & cxt, const parser::type & parser_type, source::type & source_type, const template_arg_vector & template_arguments) {
                
                parser::cursor  decl_cursor = cxt.driver->get_type_declaration(parser_type);
                parser::type    decl_type = cxt.driver->get_type (decl_cursor);
                ...
                source_type.base_struct = source::get_or_make_struct_id (cxt.source_map, decl_type.identifier);

                source::struct_info & base_struct = cxt.source_map.structs[source_type.base_struct];

                // extract semantic name
                semantic_path path = cxt.driver->get_semantic_path (decl_cursor);

                for (const parser::cursor & p : path)
                    base_struct.path.push_back (p.identifier);

                // extract template arguments
                if (template_arguments.size() > 0) {
                    for (const template_argument & ta : template_arguments) {
                        source_type.template_arguments.push_back (source::template_argument {
                            ta.identifier,
            ...
            struct type_action_context {
                parser::type    canon_type;
                source::type_id source_type_index;
                source::type &  source_type;
                bool            is_new;
            };

            type_action_context base_type_action (context & cxt, const parser::type & type) {
                parser::type canon_type = type;
            
                while (canon_type.kind != type_kind::type_kind_typedef)
                    canon_type = cxt.driver->get_canonical_type(canon_type);

                auto source_type_index = source::get_or_make_struct_id (cxt.source_map, canon_type.identifier);
                bool is_new = source_type_index == source::type_vector::nindex;
                ...
                if (is_new) {
                    ...
                    source::type & source_type = emplace_ret.second;
